...
def get_recruiter_job(
    ...
    """Return durable batch status through an unguessable capability URL.

    Firebase ID tokens are intentionally required when a recruiter job is
    created, but they are not required for subsequent status polling. Firebase
    ID tokens normally expire after about one hour, while a production batch
    containing hundreds or thousands of candidates can legitimately run for
    several hours. Re-authenticating every polling request would therefore
    couple the lifetime of a durable server-side job to a short-lived browser
    credential and could interrupt an otherwise healthy batch.

    ``job_id`` is a cryptographically random UUID generated by the backend and
    acts as a bearer capability for this read-only endpoint. The response does
    not expose the owning user ID, authentication data, uploaded CV contents or
    the original input payload. All mutating Recruiter Workspace operations
    remain protected by Firebase authentication and database ownership checks.
    """
    normalized_job_id = str(job_id or "").strip()
    try:
        parsed_job_id = uuid.UUID(normalized_job_id)
    except (ValueError, AttributeError, TypeError):
        raise HTTPException(status_code=404, detail="Recruiter job not found.")

    # Require the canonical UUID representation to avoid ambiguous aliases and
    # keep capability identifiers stable across logs, clients and databases.
    if str(parsed_job_id) != normalized_job_id.lower():
        raise HTTPException(status_code=404, detail="Recruiter job not found.")

    job = (
        db.query(RecruiterJob)
        .filter(RecruiterJob.job_id == normalized_job_id)
        .first()
    )
    if job is None:
        raise HTTPException(status_code=404, detail="Recruiter job not found.")
