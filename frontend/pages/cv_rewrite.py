...
def create_pdf_report(
    ...
    """Create the unified final CV Rewrite PDF without re-running backend analysis."""
    try:
        fields = get_rewrite_fields(result)
        bullets = fields["rewritten_bullets"]
        keywords = fields["keywords"]
        cautions = fields["cautions"]
        return build_branded_pdf_report(
            title="CV Rewrite Report",
            report_label="CV Rewrite AI",
            accent_hex=MODULE_ACCENTS["cv_rewrite"],
            subtitle="Role-aligned positioning, rewritten experience and ATS enrichment.",
            filename_hint=cv_filename,
            metrics=(
                ("Headline", "READY" if fields["headline"] else "REVIEW"),
                ("Bullets", len(bullets)),
                ("ATS keywords", len(keywords)),
                ("Cautions", len(cautions)),
            ),
            sections=(
                {"title": "Suggested Headline", "kind": "card", "content": fields["headline"], "fallback": "No headline returned."},
                {"title": "Rewritten Summary", "kind": "card", "content": fields["rewritten_summary"], "fallback": "No rewritten summary returned."},
                {"title": "Rewritten Experience", "kind": "numbered", "content": bullets, "fallback": "No rewritten bullet points returned."},
                {"title": "ATS Keywords to Add", "kind": "bullets", "content": keywords, "fallback": "No ATS keywords returned."},
                {"title": "Cautions", "kind": "bullets", "content": cautions, "fallback": "No cautions returned."},
                {"title": "Target Job Description", "kind": "text", "content": job_description},
            ),
        )
