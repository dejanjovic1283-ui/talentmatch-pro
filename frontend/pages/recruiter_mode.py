...
def create_pdf_report(result: Dict[str, Any], job_description: str) -> Optional[bytes]:
    """Create the unified final Recruiter Mode PDF without re-running ranking."""
    try:
        candidates = candidate_list(result)
        summary = str(result.get("summary") or result.get("recruiter_summary") or f"Recruiter ranking completed for {len(candidates)} candidate(s).")
        recommendations = normalize_list(result.get("recommendations"))
        ...
        sections: list[dict[str, Any]] = [
            {"title": "Recruiter Summary", "kind": "card", "content": summary},
        ]
        for item in candidates:
            candidate_summary = item["summary"] or "No candidate summary returned."
            sections.extend([
                {"title": f"#{item['rank']} {item['filename']} - {item['score']}/100 - {item['verdict']}", "kind": "card", "content": candidate_summary},
                {"title": "Strengths", "kind": "bullets", "content": item["strengths"], "fallback": "No strengths returned."},
                {"title": "Weaknesses / Gaps", "kind": "bullets", "content": item["weaknesses"], "fallback": "No weaknesses returned."},
                {"title": "Candidate Recommendations", "kind": "bullets", "content": item["recommendations"], "fallback": "No candidate recommendations returned."},
            ])
        sections.extend([
            {"title": "Overall Recommendations", "kind": "numbered", "content": recommendations, "fallback": "No overall recommendations returned."},
            {"title": "Job Description Appendix", "kind": "text", "content": job_description},
        ])
        return build_branded_pdf_report(
            title="Recruiter Mode Report",
            report_label="Recruiter Mode",
            accent_hex=MODULE_ACCENTS["recruiter_mode"],
            subtitle="Ranked candidate intelligence, evidence, gaps and recruiter recommendations.",
            metrics=(
                ("Candidates", len(candidates)),
                ("Top score", f"{top['score']}/100" if top else "N/A"),
                ("Top candidate", top["filename"] if top else "N/A"),
                ("Top verdict", top["verdict"] if top else "N/A"),
            ),
            sections=sections,
        )
