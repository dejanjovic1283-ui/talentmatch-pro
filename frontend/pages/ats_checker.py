...
def create_pdf_report(
    ...
    """Create the unified final ATS Checker PDF without re-running analysis."""
    try:
        report = extract_report_data(data)
        return build_branded_pdf_report(
            title="ATS Checker Report",
            report_label="ATS Checker",
            accent_hex=MODULE_ACCENTS["ats_checker"],
            subtitle="ATS keyword coverage, gaps and priority optimization guidance.",
            filename_hint=clean_text(cv_filename, max_chars=200),
            metrics=(
                ("ATS score", f"{report['score']}/100"),
                ("Status", report["label"]),
                ("Matched", len(report["matched_keywords"])),
                ("Missing", len(report["missing_keywords"])),
            ),
            sections=(
                {"title": "Assessment", "kind": "card", "content": report["message"]},
                {"title": "Executive Summary", "kind": "card", "content": report["summary"], "fallback": "No summary returned."},
                ...
                {"title": "Job Description Appendix", "kind": "text", "content": clean_text(job_description, max_chars=PDF_JOB_DESCRIPTION_CHARACTERS)},
            ),
        )
    except Exception as exc:
        LOGGER.exception("ATS PDF generation failed.", extra={"event": "ats_pdf_generation_failed", "error_type": type(exc).__name__})
        st.error("The ATS PDF report could not be generated. TXT export remains available.")
        return None
