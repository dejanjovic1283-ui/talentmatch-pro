...
def create_pdf_report(
    ...
    try:
        ...
        def pdf_text(value: Any, *, max_chars: int = 4_000) -> str:
        ...
        def bullet_flowable(
            ...
            paragraphs = [
                Paragraph(pdf_text(value, max_chars=MAX_LIST_ITEM_CHARS), body_style)
        ...
        def section_banner(
            ...
            banner = Table(
                [[Paragraph(f"<b>{pdf_text(title, max_chars=120)}</b>", body_style)]],
                colWidths=[176 * mm],
            )
            banner.setStyle(
                TableStyle(
        ...
        metric_data = [
            ...
            [
                Paragraph(f"{report['combined_score']}/100", metric_value_style),
                Paragraph(f"{report['semantic_score']}/100", metric_value_style),
                Paragraph(f"{report['keyword_score']}/100", metric_value_style),
            ],
            [
                Paragraph(pdf_text(report["verdict"], max_chars=120), verdict_style),
                Paragraph(
        ...
        story.extend(
            [
                ...
                section_banner("Matched Themes", "#ECFDF5", "#A7F3D0"),
                Spacer(1, 1.5 * mm),
                bullet_flowable(
                    report["matched_themes"],
                    "No matched themes returned.",
                ),
                section_banner("Missing Themes", "#FFF7ED", "#FED7AA"),
                Spacer(1, 1.5 * mm),
                bullet_flowable(
                ...
                section_banner("Matched Keywords", "#F0FDFA", "#99F6E4"),
                Spacer(1, 1.5 * mm),
                bullet_flowable(
                    report["matched_keywords"],
                    "No matched keywords returned.",
                ),
                section_banner("Missing Keywords", "#F8FAFC", "#CBD5E1"),
                Spacer(1, 1.5 * mm),
                bullet_flowable(
        ...
        for index, recommendation in enumerate(recommendations, start=1):
            story.extend(
                [
                    Paragraph(f"Priority {index}", subsection_style),
                    Table(
                        [
                            [
        ...
        LOGGER.exception(
            "Semantic PDF generation failed.",
            extra={
                "event": "semantic_pdf_generation_failed",
                "error_type": type(exc).__name__,
            },
        )
        st.error(
            "The Semantic Match PDF could not be generated. "
            "TXT export remains available. Please refresh the page and try again."
        )
        return None
