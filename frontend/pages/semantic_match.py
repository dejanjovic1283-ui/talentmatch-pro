...
def create_pdf_report(
    ...
    """Create the unified final Semantic Match PDF without re-running analysis."""
    try:
        report = extract_report_data(data)
        return build_branded_pdf_report(
            title="Semantic Match Report",
            report_label="Semantic Match",
            accent_hex=MODULE_ACCENTS["semantic_match"],
            subtitle="Semantic alignment, keyword coverage and recruiter-ready recommendations.",
            filename_hint=clean_text(cv_filename, max_chars=200),
            metrics=(
                ("Overall", f"{report['combined_score']}/100"),
                ("Semantic", f"{report['semantic_score']}/100"),
                ("Keyword", f"{report['keyword_score']}/100"),
                ("Status", report["verdict"]),
            ),
            sections=(
                {"title": "Executive Recruiter Summary", "kind": "card", "content": report["summary"]},
                {"title": "Matched Themes", "kind": "bullets", "content": report["matched_themes"], "fallback": "No matched themes returned."},
                {"title": "Missing Themes", "kind": "bullets", "content": report["missing_themes"], "fallback": "No missing themes returned."},
                {"title": "Matched Keywords", "kind": "bullets", "content": report["matched_keywords"], "fallback": "No matched keywords returned."},
                {"title": "Missing Keywords", "kind": "bullets", "content": report["missing_keywords"], "fallback": "No missing keywords returned."},
                {"title": "Priority Recommendations", "kind": "numbered", "content": report["recommendations"], "fallback": "No recommendations returned."},
                {"title": "Job Description Appendix", "kind": "text", "content": clean_text(job_description, max_chars=PDF_JOB_DESCRIPTION_CHARS)},
            ),
        )
    except Exception as exc:
        LOGGER.exception("Semantic PDF generation failed.", extra={"event": "semantic_pdf_generation_failed", "error_type": type(exc).__name__})
        st.error("The Semantic Match PDF could not be generated. TXT export remains available.")
        return None
