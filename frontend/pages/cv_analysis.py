...
def create_pdf_report(result: Dict[str, Any], cv_filename: str, job_description: str) -> Optional[bytes]:
    strengths = normalize_list(result.get("strengths") or result.get("matched_skills"))
    weaknesses = normalize_list(result.get("weaknesses") or result.get("missing_skills"))
    recommendations = normalize_list(result.get("recommendations"))
...
def analyze_resume(uploaded_file, job_description: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 403:
            st.error("CV Analysis is currently blocked by backend permissions.")
            return None
...
def render_analysis_result(result: Dict[str, Any], cv_filename: str, job_description: str) -> None:
    score = result.get("score", 0)
    summary = result.get("summary") or result.get("analysis") or ""
    strengths = normalize_list(result.get("strengths") or result.get("matched_skills"))
    weaknesses = normalize_list(result.get("weaknesses") or result.get("missing_skills"))
...
if analyze_clicked:
    ...
    if result:
        ...
        st.session_state["cv_analysis_filename"] = uploaded_file.name
        st.session_state["cv_analysis_job_description"] = job_description.strip()

if st.session_state.get("cv_analysis_result"):
    render_analysis_result(
        result=st.session_state["cv_analysis_result"],
        cv_filename=st.session_state["cv_analysis_filename"],
        job_description=st.session_state["cv_analysis_job_description"],
    )