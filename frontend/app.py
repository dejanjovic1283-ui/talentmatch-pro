...
with col1: st.metric("AI CV Match", "GPT Powered")
with col2: st.metric("ATS Scanner", "Built In")
with col3: st.metric("Semantic Match", "Enabled" if is_pro else "Upgrade")
with col4: st.metric("Recruiter Mode", "Enabled" if is_pro else "Upgrade")

st.divider()

# Информација о слободним анализама
if not is_logged_in():
...
def analyze_cv(uploaded_file, job_description):
    ...
    try:
        response = api_post("/analyze-resume", data=data, files=files, timeout=180)
...
if st.button("Analyze CV", use_container_width=True, disabled=not (is_logged_in() and uploaded_file and job_description_filled)):
    with st.spinner("Analyzing CV with AI..."):
        result = analyze_cv(uploaded_file, job_description)
    if result:
        result["cv_filename"] = uploaded_file.name if uploaded_file else st.session_state.get("last_uploaded_name", "resume.pdf")
...
if result:
    ...
    verdict = result.get("verdict") or ("Strong Match" if score >= 80 else "Good Match" if score >= 60 else "Weak Match")
    ...
    metrics[2].metric("CV file", result.get("cv_filename", st.session_state.get("last_uploaded_name", "resume.pdf")))
    ...
    txt_lines = [
        ...
        "="*40, "",
        f"Score: {score}/100", f"Verdict: {verdict}",
        "", "Summary", "-"*20, result.get("summary", ""),
        "", "Strengths", "-"*20,
    ]
    ...
    with download_col2:
        if is_pro:
            # Dummy PDF placeholder; real generation via backend endpoint.
            if st.button("📄 Generate PDF Report", use_container_width=True):
                st.info("PDF report generation via backend is not yet implemented in this version.")
        else:
    ...
    with col1:
        ...
        if not strengths: st.write("No strengths returned.")
    with col2:
        st.markdown("## ❌ Missing Skills")
        for item in weaknesses: st.markdown(f"- {item}")
        if not weaknesses: st.write("No missing skills returned.")

    st.markdown("## 💡 Recommendations")
    for item in recommendations: st.markdown(f"- {item}")
    if not recommendations: st.write("No recommendations returned.")